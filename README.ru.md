...
9. **ModernBERT-base**, со средним значением около 0.8 для релевантных текстов, выглядит относительно устойчивой моделью. В данном тесте модель пропустила только несколько терминов, что указывает на её способность справляться с опечатками. Тем не менее, неоднозначность прошлых тестов ставит под вопрос её общую адекватность.
