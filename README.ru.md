...
Выбор между моделями зависит от задач. Например, если требуется строгое разделение запросов, подходящей будет модель от OpenAI. Для более мягкого разделения можно рассмотреть Voyage или ModernBERT-gte. Главное, чтобы модель не вела себя как ModernBERT в приведённой визуализации, где диагональ совсем не выделяется, а все запросы выглядят одинаково релевантными.
...
![image](https://github.com/user-attachments/assets/c2543f75-a5f9-4836-bdc8-9a34f6c46e42)
...
8. **ModernBERT**: Модель ведёт себя неадекватно: тексты категории _Gestational diabetes_ имеют среднюю близость к _LADA_ выше, чем к «своему» названию категории. Средняя близость к _LADA_ превышает 0.8, тогда как к _Gestational diabetes_ она составляет около 0.7. Такое поведение говорит о том, что модель не способна корректно различать категории.
...
7. **ModernBERT-gte (gte-modernbert-base):** в прошлом тесте среднее показывала значение сходства для релевантных текстов около 0.7, и здесь модель выдаёт сопоставимые результаты. Большинство терминов с опечатками получили оценки, соответствующие ожидаемым для релевантных пар. Модель совсем не справилась только с опечаткой в слове "therapy", а в остальных случаях она уверенно различила слова с опечатками.

9. **ModernBERT**, со средним значением около 0.8 для релевантных текстов, выглядит относительно устойчивой моделью. В данном тесте модель пропустила только несколько терминов, что указывает на её способность справляться с опечатками. Тем не менее, неоднозначность прошлых тестов ставит под вопрос её общую адекватность.
...
| Термин                            | OpenAI                                    | Voyage                                 | Alibaba                                | Jina                                   | BioBERT                                | MedEmbed                              | ModernBERT-GTE                         | ModernBERT                            |
|---------------------------------|-------------------------------------------|----------------------------------------|----------------------------------------|----------------------------------------|----------------------------------------|----------------------------------------|----------------------------------------|----------------------------------------|
