...
1. **OpenAI** (text-embedding-3-large) demonstrates consistent performance across most terms, including difficult cases like "therapy → therpay" (0.71), which confused other models. While its similarity scores are lower than those of some other models, this does not indicate failure. In the previous test, OpenAI's average similarity for relevant documents was ~0.45. Since all typo-induced similarities remain above this threshold, the model appears to distinguish typos from fully correct terms while still recognizing their intended meaning.
