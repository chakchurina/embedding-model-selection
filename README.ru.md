...
Выбор между моделями зависит от задач. Например, если требуется строгое разделение запросов, подходящей будет модель от OpenAI. Для более мягкого разделения можно рассмотреть Voyage или Alibaba. Главное, чтобы модель не вела себя как ModernBERT в приведённой визуализации, где диагональ совсем не выделяется, а все запросы выглядят одинаково релевантными.
