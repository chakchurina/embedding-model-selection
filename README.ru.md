...
Мы намеренно включили в анализ ModernBERT, базовую модель, которая не обучалась на задаче sentence similarity. Мы сделали это, чтобы продемонстрировать, как не должен работать векторный поиск. На её примере мы увидим, как выглядят графики для необученной модели и почему использование неподходящего эмбеддера может привести к хаотичным и бессмысленным результатам. Kudos авторам модели — она большое достижение, просто нам важно показать и отрицательный пример.

Мы включили в анализ ModernBERT, базовую языковую модель, не обученную на задачу sentence similarity. Мы выбрали её намеренно, чтобы наглядно показать, как ведет себя эмбеддер, не адаптированный к поисковым задачам. На её примере мы покажем, как выглядят векторные представления, не улавливающие семантическую близость. Так как ModernBERT изначально предназначен не для поиска, а для дообучения, то его результаты помогут понять, как выглядят результаты при использовании неподходящей модели.
...
Мы работаем в фармацевтической индустрии и для нас это отличная отправная точка. Но если ваша область, например, legaltech, BEIR оказывается не так релевантен. 
